...
function calc() {
  ...
  function initLocalSettings(selector, activeClass) {
    //установка классов активности кнопок в соответсвии с хранилищем
  ...
  function getDynamicInfirmation(selector) {
    ...
    document.querySelector('#constitution').insertAdjacentElement('afterend', messageIfNotNumber);
...
function cards() {
  ...
  var Menu = /*#__PURE__*/function () {
    function Menu(src, alt, title, descr, price, parentSelector) {
  ...
  Object(_services_services__WEBPACK_IMPORTED_MODULE_3__["getResource"])('http://localhost:3000/menu').then(function (data) {
    ...
    data.forEach(function (_ref) {
      var img = _ref.img,
          ...
          descr = _ref.descr,
          price = _ref.price;
      //дестриктуризация объектов карточек на сервере
...
function forms(formSelector, timerModalWindow) {
  ...
  forms.forEach(function (item) {
    //add function postData for each form
    bindPostData(item);
  });

  function bindPostData(form) {
    //create f, witch will send formData to server
    form.addEventListener('submit', function (e) {
      e.preventDefault(); //cancel reboot page after submit form

      var statusMessage = document.createElement("img"); //message for user

      statusMessage.src = message.loading;
      statusMessage.style.cssText = "\n            display: block;\n            margin: 0 auto;\n        ";
      form.insertAdjacentElement('afterend', statusMessage); //add div with message after form

      var formData = new FormData(form); //arg form from func postData (FormData is spec.obj.)

      var json = JSON.stringify(Object.fromEntries(formData.entries())); //get norm object from FormData obj

      Object(_services_services__WEBPACK_IMPORTED_MODULE_9__["postData"])("http://localhost:3000/requests", json) //use fetch
      .then(function (data) {
        console.log(data);
  ...
  function showThanksModal(message) {
    ...
    Object(_modal__WEBPACK_IMPORTED_MODULE_8__["showModalWindow"])('.modal', timerModalWindow);
...
function modal(triggerSelector, modalSelector, timerModalWindow) {
  //modal
  var modalButtons = document.querySelectorAll(triggerSelector),
      modalWindow = document.querySelector(modalSelector);
  modalButtons.forEach(function (btn) {
    btn.addEventListener('click', function () {
      return showModalWindow(modalSelector, timerModalWindow);
    });
  });
  modalWindow.addEventListener("click", function (e) {
    //закрытие при клике на подложку и крестик
...
function slider(_ref) {
  ...
  if (slides.length < 10) {
    ...
    current.textContent = "0".concat(position);
  } else {
  ...
  next.addEventListener("click", function () {
    if (offset == deleteNotDigits(width) * (slides.length - 1)) {
      //если докрутил до последнего слайда 
      offset = 0; //возврат к первому
    } else {
      offset += deleteNotDigits(width); //если нет, то добавляю на ширину слайда
    }

    slidesFiels.style.transform = "translateX(-".concat(offset, "px)"); //прокрутка слайдов
  ...
  prev.addEventListener("click", function () {
    if (offset == 0) {
      //если докрутил до первого слайда
      offset = deleteNotDigits(width) * (slides.length - 1); //возврат к последему
    } else {
      offset -= deleteNotDigits(width); //если нет, то добавляю на ширину слайда
    }

    slidesFiels.style.transform = "translateX(-".concat(offset, "px)");
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  tabsParent.addEventListener('click', function (event) {
    ...
    if (target && target.classList.contains(tabsSelector.slice(1))) {
      //удаляю первый символ (точку)
      tabsButtons.forEach(function (item, i) {
        if (target == item) {
          hideTabContent();
          showTabContent(i);
        }
...
window.addEventListener('DOMContentLoaded', function () {
  var timerModalWindow = setTimeout(function () {
    return Object(_modules_modal__WEBPACK_IMPORTED_MODULE_1__["showModalWindow"])('.modal', timerModalWindow);
  }, 600000);
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 2:
            ...
            throw new Error("Couldn't getch ".concat(url, ", status: ").concat(res.status));
